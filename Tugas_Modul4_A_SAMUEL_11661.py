...
def scatter(model, model_name, data, new_point, features, color_scale, title):
    clusters = model.fit_predict(data[features]) #untuk memprediksi cluster untuk setiap titik pada data menggunakan m
    data[f"{model_name}_Cluster"] = clusters

    #menentukon cluster untuk titik baru
...
st.set_page_config(
    page_title="11661 - Unsupervised Learning",
    page_icon="lil",
    layout="wide",
...
if uploaded_file is not None:
    ...
    st.markdown("<h1 style='text-align: center; '>Unsupervised Learning - SAMUEL</h1>", unsafe_allow_html=True)
